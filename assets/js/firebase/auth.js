...
export async function resetPassword(email) {
  try {
    ...
    return {
      success: true,
      message:
        "Link đặt lại mật khẩu đã được gửi tới email của bạn. Hãy kiểm tra hộp thư (cả mục Spam).",
    };
  } catch (error) {
    return { success: false, message: getErrorMessage(error.code) };
  }
...
export async function resendVerification(email, password) {
  try {
    ...
    if (user.emailVerified)
      return { success: false, message: "Tài khoản này đã được xác thực rồi!" };
    ...
    return {
      success: true,
      message:
        "Đã gửi lại email xác thực. Vui lòng kiểm tra hộp thư (cả mục Spam).",
    };
  } catch (error) {
    return { success: false, message: getErrorMessage(error.code) };
  }
